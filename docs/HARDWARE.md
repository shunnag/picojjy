...
| 本体(Pico W、Wi-Fi 接続状態) | 平均 30〜50mA ≒ 0.15〜0.25W(環境により変動) |
| Wi-Fi 送受信時のピーク | 瞬間的に 200mA 程度 |

本体の消費は、初回の NTP 同期後に Wi-Fi を省電力モード
(PM_PERFORMANCE)へ自動で切り替えることで抑えています(NTP 再同期は
毎時 1 回・0.25 秒だけなので、常時フルパワーで待ち受ける必要が
ありません)。アンテナ駆動分は本体の消費に比べて誤差程度です。
電源はスマホ用の 5V/500mA 以上の USB アダプタで十分で、1 ヶ月連続
稼働させても電力量は約 0.15kWh(電気代で数円程度)です。
...
- 消費の目安: 送信中 15〜25mA(Wi-Fi オフ+CPU 48MHz 時。NTP 同期中は
  60〜80mA)、lightsleep 待機中 2mA 前後。デフォルトのスケジュール
  (1 日 2 回×30 分)では約 60〜70mAh/日となり、単3 アルカリ×3 本
  (約 2000mAh)で約 1 ヶ月が見込めます。
- `STATUS_LED = False` にすると LED の消費(約 1mAh/日、1 日の消費量の
  1〜2% 程度)も削減できます。受信トラブルの調査時は点滅確認
  (「うまく受信しないとき」の 3)のため一時的に戻してください。
