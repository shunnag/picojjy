...
3. **待機中** — Wi-Fi を完全に切り、lightsleep(数 mA)で待機します。
   ...
   長時間スリープしても送信時刻の精度は保たれます。
   起床後に NTP 同期できなかった場合、そのウィンドウの送信は行いません
   (不正確な時刻を送信しないための安全側の動作です)。

消費電力の目安は、送信中 60〜80mA・待機中 2mA 前後です。デフォルト設定なら
1 日あたり約 120mAh となり、容量 2000mAh 程度の電池で 2 週間強の連続運用が
見込めます(電池の接続方法は [HARDWARE.md](HARDWARE.md) の「電池での駆動」参照)。
